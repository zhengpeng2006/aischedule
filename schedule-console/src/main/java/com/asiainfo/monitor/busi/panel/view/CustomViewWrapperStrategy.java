...
public class CustomViewWrapperStrategy implements IViewWrapperStrategy {

	private static transient Log log = LogFactory.getLog(CustomViewWrapperStrategy.class);
	...
	public String wrapperBody(TaskRtnModel rtnModel, IPanel panel)
		...
		if (rtnModel == null || rtnModel.getRtns() == null || rtnModel.getRtns().size() < 1)
			return null;
		IPanel currentPanel=(IPanel)((ICustomPanel)panel).getChildPanel(rtnModel.getIdCode());
		if (currentPanel==null)
			throw new Exception("没有找到子面板["+rtnModel.getIdCode()+"]");
		StringBuilder sb = new StringBuilder("<chart id=\""+panel.getPanel_Id()+"\" name=\""+currentPanel.getPanel_Name()+"\" typeId=\""+rtnModel.getIdCode()+"\" viewType=\""+ currentPanel.getView_Type() + "\">");
		try {
